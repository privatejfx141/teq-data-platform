...
public class errorCheck {
     ...
     */
    public static ArrayList<ArrayList<String>> parseForDB(File template) throws InvalidFormatException, IOException {
        Workbook workbook;
    ...
    public static void errorChecking() throws InvalidFormatException,
                                              IOException {
        Workbook workbook;
        workbook = new XSSFWorkbook(
                new File("/cmshome/sarranch/eclipse_workspace2/inputChecking" +
                         "/src/inputCheck/iCARE_template.xlsx"));
        Sheet sheet = workbook.getSheetAt(0);
        DataFormatter dataFormatter = new DataFormatter();
            ...
            for (int cellNum = 0; cellNum < lastColumn; cellNum++) {
                ...
                if (row.getRowNum() == 2) {
                    ...
                    if (map.get(headerArray.get(cellNum)) == null || input.isEmpty()
                        ...
                        // map.get(headerArray.get(cellNum)) + " header " +
                        // headerArray.get(cellNum));
                        cells.add(input);
                    } else {
                        System.out.println("Error, Column " + cellNum + " Row"
                                           + " " + row.getRowNum()
                                           + " does not contain an allowed " +
                                           "value: " + input + " and expected: "
    ...
    public static HashMap<String, ArrayList<String>> createAllowedValues() throws InvalidFormatException, IOException {
        ...
        workbook = new XSSFWorkbook(new File(
                "/cmshome/sarranch/eclipse_workspace2/inputChecking/src" +
                "/inputCheck/New_iCARE_Template_Comb_with_Examples.xlsx"));
