...
public class U1DatabaseUnitTest {
    ...
    static Client createDummy() {
        ...
        Client client = clientBuilder.setId(Integer.parseInt("0"))
                .setIdType(1)
                .setBirthDate(ExcelDriver.fixDate("1998-04-05", "yyyy-MM-dd"))
	...
	void testAddress() {
        ...
        String streetRepr = String.format("%d %s %s %s %s %s", streetNumber, streetName, streetType, streetDirection, city, province);
        if (unitNumber != -1) {
        ...
        String rstreetRepr = String.format("%d %s %s %s %s %s", rstreetNumber, rstreetName, rstreetType, rstreetDirection, rcity, rprovince);
        if (runitNumber != -1) {
