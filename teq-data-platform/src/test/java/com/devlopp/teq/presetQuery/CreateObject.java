...
public class CreateObject {
    ...
    public static int createClientWithLanguage(String language) {
        ...
        Client client = clientBuilder.setId(clientIdSt).setIdType(1)
                .setBirthDate(ExcelDriver.fixDate("1965-09-13", "yyyy-MM-dd")).setPhoneNumber("1234567890")
                .setEmailAddress("test@gmail.com").setAddress(address).setLanguage(language)
                .setConsent(true).create();
    ...
    public static void createDifferentServices() {
        ...
        Assessment assessment = (Assessment) assessmentBuilder.setStartDate("2018-4-05").setLanguageGoal("Goal").setOtherGoal("Other")
        		...
        		.setFindEmployment("2015-04-03", "4", "4", "true").setIncrease(tempMap).setNonIRCCServices(tempList).setId(4).create();
        DatabaseInsertHelper.insertAssessment(assessment);
        
        IOrientationBuilder orientationBuilder = new OrientationBuilder();
        Orientation orientation = (Orientation) orientationBuilder.setServiceReceived("Service").setTotalLength("24").setLengthHours(45)
        		.setLengthMinutes(54).setNumberOfClients("200").setEndDate("2019-03-28").create();
        DatabaseInsertHelper.insertOrientation(orientation);
        
        IEmploymentBuilder employmentBuilder = new EmploymentBuilder();
        Employment employment = (Employment) employmentBuilder.setRegistration(true).setReferralDate("").setReferralTo("").setEmploymentStatus("")
        		.setEducationStatus("").setOccupationCanada("").setOccupationIntended("").setInterventionType("").setTimeSpentHours(1).setTimeSpentMinutes(1)
        		.create();
